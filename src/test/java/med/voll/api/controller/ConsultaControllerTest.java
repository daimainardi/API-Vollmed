...
class ConsultaControllerTest {
    ...
    @Test
    @DisplayName("Deveria devolver código http 20 quando informações estão válidas")
    @WithMockUser
    void agendar_cenario2() throws Exception {
        ...
        var dadosDetalhamento = new DadosDetalhamentoConsultaDTO(null, 3l, 6l, data);
        when(agendaDeConsultasService.agendar(any())).thenReturn(dadosDetalhamento);

        var response = mvc.perform(post("/consultas")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(dadosAgendamentoConsultaJson.write(
                                new DadosAgendamentoConsultaDTO(3l, 6l, data, especialidade)
                        ).getJson())
                )
